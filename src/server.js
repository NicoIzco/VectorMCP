...
<section class="card">
  ...
  <div class="grid"><div class="col-8"><input id="queryInput" placeholder="manage tasks" /></div><div class="col-4"><button id="searchBtn">Search</button></div></div>
...
sourceList.innerHTML=allSources.map((s)=>{const id=s.path||s.url;const cls=s.type==='file'?'t-blue':(s.type==='repo'?'t-green':'t-purple');const icon=s.type==='file'?'📁 File':(s.type==='repo'?'🔗 Repo':'🌐 Web');const stat=s.status==='error'?'❌ Error':(s.status==='syncing'?'🔄 Syncing...':'✅ Synced');return '<div class="source"><div class="row"><span class="tag '+cls+'">'+icon+'</span><strong title="'+esc(id)+'">'+esc(truncate(id,68))+'</strong><span class="tag">'+esc(s.category||'general')+'</span></div><div class="row muted"><span>'+rel(s.lastSync)+'</span><span>'+s.toolCount+' tools</span><span>'+stat+'</span></div>'+(s.error?'<details><summary class="muted">Error details</summary><div style="color:var(--accent-red)">'+esc(s.error)+'</div></details>':'')+'<div class="actions" style="margin-top:8px"><button class="icon" onclick="syncOne(\''+esc(id)+'\', this)">🔄</button><button class="icon" data-id="'+esc(id)+'" onclick="confirmDelete(this)">🗑️</button></div></div>'}).join('');}
...
addSourceBtn.onclick=addSource;searchBtn.onclick=runSearch;categoryFilter.onchange=applyFilters;sortBy.onchange=applyFilters;queryInput.addEventListener('keydown',(e)=>{if(e.key==='Enter')runSearch();});
